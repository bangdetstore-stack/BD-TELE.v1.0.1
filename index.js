...
bot.use(async (ctx, next) => {
    try {
        if (ctx.from) {
            ...
            console.log(`${cG}│${cY}➤From: ${cY}Telegram Bot ${process.env.STORE_NAME || 'Diera Store'}${cZ}`);
            console.log(`${cG}├────────────────────────────────${cZ}`);
            console.log(`${cG}│${cZ}`);
            console.log(`${cG}├────────────────────────────────${cZ}`);
            console.log(`⧉ ${cW}${process.env.STORE_NAME || 'Diera Store'}${cZ}`);
            console.log();
            console.log(`${cG}+-------------------------------+${cZ}`);
        }
    } catch(e) {}
...
bot.action('menu_utama', async (ctx) => {
    ...
    // Coba edit teks biasa
    try {
        await ctx.editMessageText(text, opts);
        return ctx.answerCbQuery();
    } catch (e) { }

    // Kalau pesan sebelumnya adalah foto (QR), edit captionnya dulu gagal → hapus + kirim baru
    try {
...
// Produk detail - pattern: produk_<id>
...
// Cek status pembayaran: cek_pay_<orderId>_<amount>
bot.action(/^cek_pay_(.+)_(\d+)$/, (ctx) => {
    cekStatusPayment(ctx, ctx.match[1], ctx.match[2]);
});

// Batalkan pembayaran: batal_pay_<orderId>_<amount>
...
// Pilih nominal: dep_nominal_<amount>
...
// Edit field: adm_edit_<field>_<id>
...
// Tambah akun ke stok: adm_tambah_akun_<id>
...
async function main() {
    ...
    const server = app.listen(WEBHOOK_PORT, () => {
        ...
        console.log(`   URL Webhook (Public): http://apps1.vynzzhost.com:${WEBHOOK_PORT}/webhook/pakasir`);
    });
    server.on('error', (err) => {
