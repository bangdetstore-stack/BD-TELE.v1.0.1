...
const { getProducts, formatHarga } = require('./products');
...
async function beliDenganSaldo(ctx, productId) {
    ...
    if (db.hasPendingTransaction(userId)) {
    ...
    try {
        ...
        if (!akunDetail) {
            return ctx.reply('❌ Maaf, stok baru saja habis dibeli orang lain.');
        }
    ...
    if (akunDetail) {
        ...
        successText += `_Admin akan segera mengirimkan detail akun._\n\n`;
    }
    ...
    successText += `_Terima kasih sudah belanja di ${escMd(STORE_NAME)}\!_`;

    const keyboard = Markup.inlineKeyboard([
        [Markup.button.callback('🛍️ Beli Lagi', 'menu_produk'), Markup.button.callback('📋 Riwayat', 'menu_riwayat')],
        [Markup.button.callback('🏠 Beranda', 'menu_utama')]
    ]);

    // ── Kirim struk sebagai pesan PERMANEN (tidak bisa diedit/hilang)
    // Struk dikirim tanpa keyboard agar tidak bisa di-overwrite tombol lain
    try {
        // Hapus pesan lama (misal pesan detail produk)
        try { await ctx.deleteMessage(); } catch (_) { }
        // Kirim struk tanpa keyboard (permanen)
        await ctx.replyWithMarkdown(successText);
    } catch (e) {
        try { await ctx.telegram.sendMessage(userId, successText, { parse_mode: 'Markdown' }); } catch (_) { }
    }

    // Kirim pesan navigasi TERPISAH (pesan ini boleh diganti tombol)
    try {
        await ctx.telegram.sendMessage(userId,
            '_Pilih tindakan selanjutnya:_',
            {
                parse_mode: 'Markdown',
                ...Markup.inlineKeyboard([
                    [Markup.button.callback('\uD83D\uDECD\uFE0F Beli Lagi', 'menu_produk'), Markup.button.callback('\uD83D\uDCCB Riwayat', 'menu_riwayat')],
    ...
    if (LOG_CHANNEL_ID) {
        ...
        console.error('Error beli saldo:', e);
...
async function beliViaQRIS(ctx, productId) {
    ...
    try { await ctx.replyWithChatAction('upload_photo'); } catch (e) {}

    try {
        const orderId = generateOrderId('PAY');
        
        // Reservasi stok (Keep Stock) di awal
        const akunDetail = db.takeAccount(p.id, orderId);
        if (!akunDetail) {
            return ctx.reply('❌ Maaf, stok baru saja direbut orang lain.');
        }

        const result = await pakasir.createTransaction(orderId, p.harga, 'qris');

        if (!result.success) {
            db.restoreAccount(orderId); // Kembalikan stok jika gagal buat QRIS
            try {
                await ctx.editMessageText(`❌ *Gagal membuat pembayaran:*\n${escMd(result.error)}`,
                    { parse_mode: 'Markdown', ...Markup.inlineKeyboard([[Markup.button.callback('« Kembali', `produk_${p.id}`)]]) }
                );
    ...
    const keyboard = Markup.inlineKeyboard([
        [Markup.button.callback('🔄 Cek Status', `cek_pay_${orderId}_${p.harga}`), Markup.button.callback('❌ Batal', `batal_pay_${orderId}_${p.harga}`)],
        [Markup.button.callback('🏠 Beranda', 'menu_utama')]
    ]);
    ...
    const caption =
        `> 📱 *SCAN QRIS*\n> ─────────────────\n` +
        `> 🏷️ Produk: *${p.nama}*\n` +
        `> 💵 Harga : *${formatHarga(data.total_payment)}*\n` +
        `> ⏰ Expired: ${formatDate(data.expired_at)}\n> ─────────────────\n\n` +
        `_Konfirmasi otomatis setelah pembayaran sukses._`;
...
async function prosesOrderSetelahBayar(ctx, orderId) {
    ...
    // Ambil akun dari database yang sudah di-reserve saat QRIS dibuat
    const akunDetail = db.getAccountDetailByOrderId(orderId);
    const user = db.getUser(userId);
    ...
    // ── Kirim struk PERMANEN tanpa keyboard
    if (ctx.callbackQuery) {
        try { await ctx.deleteMessage(); } catch (e) { }
    }

    try {
        // Struk permanen — tidak ada inline keyboard
        await ctx.telegram.sendMessage(userId, successText, { parse_mode: 'Markdown' });
    } catch (e) {
        console.error('Gagal mengirim struk sukses:', e);
    }

    // Pesan navigasi terpisah (boleh diedit tombolnya)
    try {
        await ctx.telegram.sendMessage(userId,
            '_Pilih tindakan selanjutnya:_',
            {
...
async function batalkanPayment(ctx, orderId, amount) {
    ...
    db.deletePending(`pay_${orderId}`);
    db.restoreAccount(orderId); // Kembalikan stok yang di-reserve

    const keyboard = Markup.inlineKeyboard([
        [Markup.button.callback('🛍️ Lihat Produk', 'menu_produk'), Markup.button.callback('🏠 Beranda', 'menu_utama')]
    ]);
